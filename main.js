...
const calculateLuxValueBasedOnHeytech = function (wert) {
	...
	if (wert < 10) {
		// - LuxPrefix = 1 --> Lux-Wert n steht für   1 ... 900 Lux
		luxPrefix = 0;
		lux = wert; //  ' - LuxPrefix = 0 --> Lux-Wert n steht für 0,1 ... 0,9 Lux
	} else if (wert <= 19) {
		//  ' - LuxPrefix = 2 --> Lux-Wert n steht für   1 ... 900 kLux
		luxPrefix = 1;
		lux = wert - 9;
	} else if (wert <= 28) {
		luxPrefix = 1;
...
function createClient() {
	...
	if (this.config.ip === '' || this.config.ip === null || this.config.ip === undefined) {
		...
		client = Telnet.client(`${this.config.ip}:${this.config.port}`, {
			socket: new net.Socket({ readable: false, writable: false }),
		});
		setInterval(() => {
		...
		client
			.filter(event => event instanceof Telnet.Event.Disconnected)
			.subscribe(() => {
		...
		client.data.subscribe(data => {
			...
			if ((!readSmn && lastStrings.indexOf(START_SMN) >= 0) || lastStrings.indexOf(ENDE_SMN) >= 0) {
				...
				if (regexpResults && regexpResults.length > 0) {
					const statusStr = regexpResults[regexpResults.length - 1]
						.replace('t_sop', '')
						.replace(ENDE_SOP, '');
	...
	function writeKlima(data) {
		...
		this.getStates('sensors.*', function (err, states) {
			...
			if (
				(that.config.iTempSensor === true || that.config.humiditySensor === true || that.config.autoDetect) &&
				data[1] !== '999'
			) {
...
class Heytech extends utils.Adapter {
	...
	onReady() {
        ...
        */
		if (this.config.autoDetect === false) {
			...
			if (
				this.config.briSensor === true ||
				this.config.oTempSensor === true ||
				this.config.iTempSensor === true ||
				this.config.rainSensor === true ||
				this.config.windSensor === true ||
				this.config.alarmSensor === true
			) {
			...
			this.getForeignObjects(`${this.namespace}.groups.*`, 'group', (err, objs) => {
				if (err) {
					...
					if (nonConfigGroups.length > 0) {
						this.log.warn(
							`These groups are not configured in this.config.groups: ${nonConfigGroups.join(', ')}`,
						);
	...
	checkNewerVersion() {
		return (
			(controllerSoftwareVersion[0] === '8' && controllerSoftwareVersion >= '8.027o') ||
		...
		return memoizeDebounce(async () => {
			...
			if (ziel === 100) {
				...
				if (this.checkVersionCanSetPercentage()) {
					...
					while (
						(direction === 'down' && aktuellePosition > ziel) ||
						(direction === 'up' && aktuellePosition < ziel)
					) {
